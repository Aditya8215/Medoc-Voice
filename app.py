...
from scripts import doctor_dictation # Import for getting language keys
...
with st.sidebar:
    ...
    # <-- UPDATE: Removed Patient Name, only User Name is kept -->
    st.subheader("User Details")
    st.text_input("User Name (Required to Save)", key="user_name_input", placeholder="Enter your full name")
    st.divider()
...
if selected == "Transcription":
    ...
    with col_c:
        language = st.selectbox(
            ...
            options=list(doctor_dictation.keys())
        )
    st.markdown("---")
    ...
    with script_col:
        ...
        if st.button("Generate New Dictation Script", use_container_width=True):
            with st.spinner("Selecting a script..."):
                generated_text = generate_medical_script(dictation_type, language)
        ...
        with recorder_col:
            ...
            if st.session_state.get("needs_transcription"):
                with st.spinner("Auto-transcribing audio..."):
                    if GEMINI_AVAILABLE:
                        path_to_transcribe = noisy_recording_path if os.path.exists(noisy_recording_path) else recorded_audio_path
            ...
            with b_col2:
                if st.button("New Recording", use_container_width=True, type="primary"):
                    ...
                    keys_to_clear = ["transcription", "result", "script", "upload_complete", "cloudinary_url", "cloudinary_public_id", "needs_transcription", "needs_prescription_generation", "script_json"]
                    for key in keys_to_clear:
                        st.session_state.pop(key, None)
    ...
    with st.expander("📁 Or Upload an Existing Audio File..."):
        ...
        if uploaded_file is not None:
            ...
            if st.session_state.get('last_uploaded_file_id') != current_file_id:
                keys_to_clear = ["transcription", "result", "script", "upload_complete", "cloudinary_url", "cloudinary_public_id", "upload_dir", "needs_transcription", "needs_prescription_generation", "script_json"]
                for key in keys_to_clear:
                    st.session_state.pop(key, None)
...
if "transcription" in st.session_state:
        ...
        with st.spinner("Auto-generating prescription..."):
            if GEMINI_AVAILABLE:
                ...
                if isinstance(result, dict) and "error" in result:
                    ...
                    # <-- UPDATE: Validation for User Name only -->
                    user_name = st.session_state.get("user_name_input", "").strip()
                    
                    if not user_name:
                        st.error("⚠️ Please enter the User Name in the sidebar to save.")
                    else:
                        # Add user name to the prescription JSON
                        st.session_state.result['doctorUsername'] = user_name
                        
                        audio_url = st.session_state.get("cloudinary_url")
                        if audio_url:
                            with st.spinner(f"Saving prescription..."):
                                mongo_result = upload_prescription_to_mongodb(
                                    prescription_data=st.session_state.result, 
                                    audio_url=audio_url,
                                    original_script=st.session_state.get("script", ""),
                                    original_prescription=st.session_state.get("script_json", {}),
                            ...
                            if "inserted_id" in mongo_result:
                                st.info(f"Prescription saved successfully by {user_name}.")
    ...
    if "script" in st.session_state and st.session_state.script and "transcription" in st.session_state:
