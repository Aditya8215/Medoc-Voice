...
import time
# --- NEW IMPORTS for the robust audio handling ---
from pydub import AudioSegment
# We no longer need scipy.io.wavfile
...
def add_noise_to_audio(input_path, output_path, noise_level=0.05):
    ...
    try:
        # Load the audio file using pydub (handles inconsistent headers gracefully)
        sound = AudioSegment.from_wav(input_path)
        # Get audio data as a numpy array
        samples = np.array(sound.get_array_of_samples())
        # Generate white noise, scaled to the audio's amplitude
        # sound.max is the highest possible amplitude for the sound's bit depth
        noise = np.random.normal(0, sound.max * noise_level, len(samples))
        # Add noise to the samples
        noisy_samples = samples + noise
        # Clip the data to prevent overflow and ensure it's within the valid range
